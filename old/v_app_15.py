...
with tab3:
    if not st.session_state.data_quiz:
        ...
        # 👇👇👇 === 新增布局开始 === 👇👇👇
        # 使用两列布局，把提示语和按钮分开
        col_tip, col_reset = st.columns([3, 1])
        
        with col_tip:
            st.caption("🎯 提示：点击右上角 [再来一组] 可生成新题")
            
        with col_reset:
            # ✨ 这里就是你要的功能 ✨
            # 逻辑：只增加版本号 -> 触发 Rerun -> Radio Key 改变 -> 选项重置
            if st.button("🧹 重置选项 (Retry)", use_container_width=True):
                st.session_state.quiz_version += 1
                st.rerun()
        # 👆👆👆 === 新增布局结束 === 👆👆👆
        ...
        for i, q in enumerate(st.session_state.data_quiz['quizzes']):
            ...
            # Key 逻辑保持不变
            unique_key = f"q_{st.session_state.session_id}_{i}_v{st.session_state.quiz_version}"
...
with tab4:
    ...
    with c1:
        st.metric("📅 今日待复习 (Due Today)", f"{due_count} 个")
    with c2:
        st.metric("🏆 已完全掌握 (Mastered)", f"{mastered_count} 个")
    with c3:
        # 刷新按钮
        if st.button("🔄 刷新队列"):
    ...
    if not st.session_state.review_queue:
        ...
        if st.session_state.review_index >= len(st.session_state.review_queue):
            ...
            if not st.session_state.show_card_back:
                # 阶段 1: 翻面
                with col_b2:
                    if st.button("🔍 翻看背面", use_container_width=True):
                        st.session_state.show_card_back = True
                        st.rerun()
            else:
                # 阶段 2: 判定
                with col_b1:
                    if st.button("❌ 忘了 (Reset)", use_container_width=True):
                        # 降级逻辑
                        new_box, next_date = st.session_state.db.process_review(current_word['id'], box_lv, False)
                        
                        st.toast(f"别灰心！已重置回 Box 1，明天继续复习。", icon="💪")
                        st.session_state.review_index += 1
                        st.session_state.show_card_back = False
                        st.rerun()
                        
                with col_b3:
                    if st.button("✅ 记得 (Upgrade)", type="primary", use_container_width=True):
                        # 升级逻辑
                        ...
                        if new_box > 5:
                            ...
                            st.toast(f"升级成功！晋升至 Box {new_box}，下次复习: {next_date}", icon="📅")
