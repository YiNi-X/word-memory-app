...
# 计算文字中心位置
# 外环文字中心：外半径和内半径的中间值
labeldistance_outer = radius_inner + width_outer / 2  # 0.55 + 0.225 = 0.775
# 内圆文字中心：内半径的一半
labeldistance_inner = radius_inner / 2 # 0.55 / 2 = 0.275
...
wedges1, texts1 = ax1.pie(outer_sizes, labels=outer_labels, radius=radius_outer, colors=outer_colors,
                          ...
                          labeldistance=labeldistance_outer, # 设置为计算出的中心位置
                          textprops={'fontsize': 9, 'color': 'white', 'fontweight': 'bold'}) # 字体改小并设为白色，以便在深色背景上显示
...
wedges2, texts2 = ax1.pie(inner_sizes, labels=inner_labels, radius=radius_inner, colors=cmap_inner,
                          ...
                          labeldistance=labeldistance_inner, # 设置为计算出的中心位置
                          textprops={'fontsize': 11, 'color': 'white', 'fontweight': 'bold'})

ax1.set_title("水面宏观垃圾成分分析", fontsize=16, pad=20,y=0)

# --- 右图：实心饼图文字居中 ---
# 半径参数
radius_water = 1.0
# 计算文字中心位置
labeldistance_water = radius_water / 2 # 0.5
...
wedges3, texts3 = ax2.pie(water_sizes, labels=water_labels, radius=radius_water, colors=water_colors_blue,
                          ...
                          labeldistance=labeldistance_water, # 设置为中心位置
                          textprops={'fontsize': 12, 'color': 'white', 'fontweight': 'bold'}) # 字体设为白色

ax2.set_title("水体微观污染监测要素占比", fontsize=16, pad=20,y=0)

plt.tight_layout()
plt.savefig('water_pollution_analysis_centered_labels.png', dpi=300)