...
with tab1:
    if not st.session_state.data_article:
        ...
        if st.button("🚀 启动阅读引擎"):
            with st.spinner("正在编织故事线..."):
                try:
                    res = st.session_state.ai.generate_article(st.session_state.current_words)
                    st.session_state.data_article = res
                    # 保存到 DB
                    st.session_state.db.update_article(st.session_state.session_id, res['article_english'], res['article_chinese'])
                    st.rerun()
...
with tab2:
    if not st.session_state.data_cards:
        ...
        if st.button("🧠 启动记忆解析"):
            with st.spinner("正在解析神经突触..."):
                try:
                    res = st.session_state.ai.analyze_words(st.session_state.current_words)
                    st.session_state.data_cards = res
                    # 保存到 DB
                    st.session_state.db.save_words(st.session_state.session_id, res['words'])
                    st.rerun()
                except Exception as e:
                    st.error(f"System Failure: {e}")
    else:
...
with tab3:
    if not st.session_state.data_quiz:
        ...
        # 依赖检查
        if not st.session_state.data_article:
            st.warning("⚠️ 建议先生成文章 (System 2)，以便生成结合上下文的题目。")
        
        if st.button("⚔️ 启动实战演练"):
            with st.spinner("正在生成战场..."):
                try:
                    # 优化：提取文章上下文
                    article_context = st.session_state.data_article['article_english'] if st.session_state.data_article else None
                    
                    res = st.session_state.ai.generate_quiz(st.session_state.current_words, article_context)
                    st.session_state.data_quiz = res
                    # 保存到 DB (JSON 字符串化)
                    st.session_state.db.update_quiz(st.session_state.session_id, json.dumps(res))
                    st.rerun()
                except Exception as e:
                    st.error(f"System Failure: {e}")
    else:
        for i, q in enumerate(st.session_state.data_quiz['quizzes']):
            ...
            # 使用 callback 处理选项状态，防止页面刷新丢失选择
            choice_key = f"quiz_{st.session_state.session_id}_{i}"
            choice = st.radio("Select Option:", q['options'], key=choice_key, index=None)
