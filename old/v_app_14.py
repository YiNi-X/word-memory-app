...
# === TAB 1: 文章模块 (自动链式触发核心) ===
with tab1:
    # 场景 A: 完全没有任何数据，显示启动按钮
    if not st.session_state.data_article:
        st.info("等待指令... 神经网络处于待机状态。")
        
        if st.button("🚀 启动序列 (Start Sequence)"):
            with st.spinner("正在接收来自虚空的故事信号... (Step 1/2: Generating Article)"):
                try:
                    # 1. 请求文章
                    ...
                    # ⚠️ 关键点：文章生成完，立即 Rerun，目的是立刻把文章渲染出来给用户看
                    st.rerun()
    ...
    # 场景 B: 文章已经有了，直接渲染文章
        ...
        # --- 2. 自动检测：如果有文章但没有单词卡，自动启动后台任务 ---
        if not st.session_state.data_cards:
            # 使用 status 容器，给用户一种“后台正在处理”的感觉，不打扰阅读
            with st.status("🤖 系统后台正在分析记忆碎片...", expanded=False) as status:
                st.write("正在提取词根...")
                st.write("正在构建联想画面...")
                ...
                try:
                    ...
                    status.update(label="✅ 记忆碎片分析完成！(Tab 2 Ready)", state="complete", expanded=False)
                    # 可选：完成后再次刷新，或者就留在这里等用户自己切 Tab
                    # st.rerun() 
                except Exception as e:
...
with tab3:
    if not st.session_state.data_quiz:
        ...
        if st.session_state.data_article:
            ...
            st.warning("⚠️ 请先在 System 2 生成文章。")
    else:
        # 渲染题目 (保持不变)
